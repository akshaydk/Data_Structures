...
class binary_tree:
    ...
    def find_max_without_recursion(self,node):
        if node is None:
            return
        my_list=[]
        my_list.append(node)
        ...
        while len(my_list)!=0:
            element=my_list.pop()
            if element.get_data() > max_element:
                max_element=element.get_data()
...
if __name__=='__main__':
    ...
    max_element=new_tree.find_max_without_recursion(node)
    print max_element
